...
public class Photonmanager : MonoBehaviourPunCallbacks
{
    ...
    {
        PhotonNetwork.LocalPlayer.CustomProperties.Remove(PlayerInformation.PLYAER_RESULT);
        ...
        PhotonNetwork.LocalPlayer.CustomProperties.Remove(PlayerInformation.PLAYER_CS);

        PhotonNetwork.LocalPlayer.CustomProperties.Add(PlayerInformation.PLYAER_RESULT, DataManager.instance.result);
        PhotonNetwork.LocalPlayer.CustomProperties.Add(PlayerInformation.PLYAER_KILL, DataManager.instance.kill);
        PhotonNetwork.LocalPlayer.CustomProperties.Add(PlayerInformation.PLAYER_DEATH, DataManager.instance.death);
        PhotonNetwork.LocalPlayer.CustomProperties.Add(PlayerInformation.PLAYER_CS, DataManager.instance.cs);
        ...
        if (EndPanel.activeInHierarchy && DataManager.instance.gameOver)
        {
            ...
            for (int playerInRoom = 0; playerInRoom < room.PlayerCount; playerInRoom++)
            {
                if (PhotonNetwork.PlayerList[playerInRoom].CustomProperties.TryGetValue(PlayerInformation.PLAYER_ROOMNUMBER, out isPlayerRoomnuber))
                {
                    ...
                    {
                        if ((bool)isPlayerRed) RedPlayer_ResultPanel[(int)isPlayerRoomnuber].GetComponent<Text>().text = PhotonNetwork.PlayerList[playerInRoom].NickName;

                        if (!(bool)isPlayerRed) BluePlayer_ResultPanel[(int)isPlayerRoomnuber].GetComponent<Text>().text = PhotonNetwork.PlayerList[playerInRoom].NickName;
                    }
                }
            }
        }
    ...
    void OnApplicationQuit() =>
        StartCoroutine(LogOutUser(IDInputField.GetComponent<InputField>().text));//디비에 로그아웃
